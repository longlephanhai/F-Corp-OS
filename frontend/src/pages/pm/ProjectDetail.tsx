...
import { 
    Button, Table, Tag, Avatar, Modal, Form, 
    Input, DatePicker, Space, Typography, Card, Divider, Flex, message, Select
} from 'antd';
import { 
...
export const ProjectDetail: React.FC = () => {
    ...
    const getSprintStatus = (start: string, end: string) => {
        ...
        if (today.isAfter(endDate)) return { text: 'ĐÃ HOÀN THÀNH', color: 'green' };
    ...
    const columns = [
        {
            ...
            render: (text: string, record: any) => (
                <Space direction="vertical" size={0}>
                    <Text strong style={{ fontSize: '15px' }}>{text || 'Sprint Chưa đặt tên'}</Text>
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Text type="secondary">
                    {dayjs(record.start_date || record.startDate).format('DD/MM/YYYY')} - {dayjs(record.end_date || record.endDate).format('DD/MM/YYYY')}
                </Text>
            ),
        },
        {
            ...
            key: 'status',
            render: (_: any, record: any) => {
                const status = getSprintStatus(record.start_date || record.startDate, record.end_date || record.endDate);
        ...
        },
        {
            ...
            render: (attendant: any) => {
                ...
                let roles = [];
                try {
                    roles = typeof attendant === 'string' ? JSON.parse(attendant) : attendant;
                } catch (e) { roles = []; }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {/* --- THÔNG TIN DỰ ÁN --- */}
            <Card bordered={false} style={{ boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.03)' }} loading={loading}>
                <Flex justify="space-between" align="flex-start">
                    <Space direction="vertical" size="small">
                        <Space align="center" size="middle">
                            <Title level={2} style={{ margin: 0, color: '#1f2937' }}>{project?.name || 'Đang tải...'}</Title>
                            <Tag color={project?.status === 'active' ? 'success' : 'default'} style={{ fontWeight: 'bold' }}>
                                {project?.status?.toUpperCase() || 'N/A'}
                            </Tag>
                        </Space>
                        
                        <Space size="large" style={{ color: '#6b7280', marginTop: '8px' }}>
                            <Space><UserOutlined /> <Text type="secondary">PM: {project?.pm?.fullName || 'Chưa gán'}</Text></Space>
                            <Space><CalendarOutlined /> <Text type="secondary">{project?.start_date} → {project?.end_date}</Text></Space>
                            <Space><DollarOutlined /> <Text type="secondary">Ngân sách: Chờ cập nhật</Text></Space>
                        </Space>
                        <Text type="secondary" style={{ marginTop: '8px' }}>{project?.description}</Text>
                    </Space>
            ...
            <Modal
                ...
                width={700}
                footer={[
                    <Button key="back" onClick={() => setIsModalOpen(false)}>Hủy bỏ</Button>,
                    <Button key="submit" type="primary" loading={loading} onClick={() => form.submit()}>Chốt Tạo Sprint</Button>,
                ]}
                destroyOnClose
            >
                <div style={{ marginBottom: '24px', color: '#6b7280' }}>
                    Xác định phạm vi, thời gian và các vị trí cần thiết cho chặng tiếp theo.
                </div>
                ...
                <Form form={form} layout="vertical" onFinish={handleCreateSprint}>
                    ...
                    >
                        <Input placeholder="Ví dụ: Sprint 3 - Tính năng giỏ hàng" size="large" />
                    </Form.Item>
