...
export const EvidenceApprovalModal: React.FC<Props> = ({ open, member, onClose, onRefresh }) => {
  ...
  return (
    <Modal
      title={<span className="text-xl font-bold">Hồ sơ năng lực: {member.fullName}</span>}
      open={open}
      onCancel={onClose}
      footer={null}
      width={900}
    >
      <div className="flex flex-col md:flex-row gap-6 mt-4">
        ...
        <div className="w-full md:w-1/2 bg-gray-50 p-4 rounded-lg border border-gray-100">
          <h3 className="text-center font-semibold text-gray-700 mb-2">Ma trận kỹ năng</h3>
          <SkillRadarChart skills={member.userSkills} />
        </div>

        {/* Cột phải: Danh sách bằng chứng */}
        <div className="w-full md:w-1/2">
          <h3 className="font-semibold text-gray-700 mb-2">Danh sách Bằng chứng / Chứng chỉ</h3>
          <List
              ...
              <List.Item className="bg-white border border-gray-200 rounded-lg mb-3 p-4 block hover:shadow-md transition">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <Tag color={item.evidenceType === 'certification' ? 'purple' : 'cyan'}>
                      {item.evidenceType}
                    </Tag>
                  </div>
                  <Tag color={item.status === 'pending' ? 'orange' : item.status === 'verified' ? 'green' : 'red'}>
                ...
                <div className="mb-3">
                  ...
                  <a href={item.evidenceUrl} target="_blank" rel="noreferrer" className="text-sm text-blue-600 break-all">
                    {item.evidenceUrl}
                  </a>
                </div>
                    ...
                    {rejectingId === item.id ? (
                      <div className="flex gap-2">
                        <Input 
                          ...
                          size="small"
                        />
                        <Button type="primary" danger size="small" onClick={() => handleReject(item.id)}>Chốt</Button>
                        <Button size="small" onClick={() => setRejectingId(null)}>Hủy</Button>
                      </div>
                    ) : (
                      <div className="flex justify-end gap-2">
                        <Button size="small" danger onClick={() => setRejectingId(item.id)}>
                          Từ chối
                        </Button>
                        <Button size="small" type="primary" onClick={() => handleVerify(item.id)}>
                          Duyệt hợp lệ
                        </Button>
                      </div>
                    )}
