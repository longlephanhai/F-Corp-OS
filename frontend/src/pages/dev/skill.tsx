...
import { SprintInvitationsPanel } from "../../components/dev/SprintInvitationsPanel";
...
const SkillPage = () => {
  ...
  const stats = useMemo(() => {
    ...
    const hasDescription = skills.filter((item) =>
      safeText(item.description).trim(),
    ).length;
    ...
    },
    {
      ...
      render: (description: string) => (
        <Text
          ...
          style={{ display: "inline-block", maxWidth: 460 }}
        >
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <Flex justify="space-between" gap={16} wrap="wrap" align="flex-start">
          <div style={{ maxWidth: 760 }}>
            ...
            <Paragraph
              style={{ marginBottom: 0, maxWidth: 680, color: "#475467" }}
            >
              Trang này hiển thị danh sách kỹ năng từ backend, kèm người tạo,
              thời gian cập nhật, trạng thái và hỗ trợ tìm kiếm nhanh.
            </Paragraph>
          </div>
      ...
      <Card
        ...
        extra={
          <Text type="secondary">
            Trang {current} / {Math.max(Math.ceil(total / pageSize), 1)}
          </Text>
        }
      >
        <Table
