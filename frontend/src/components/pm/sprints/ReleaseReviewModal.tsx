...
import { Modal, Form, Input, Rate, Button, message, Divider, Avatar } from 'antd';
...
export const ReleaseReviewModal: React.FC<Props> = ({ open, userSprintId, devName, onClose, onRefresh }) => {
  ...
  return (
    <Modal
      title={<span className="text-xl font-bold text-gray-800">Đánh giá & Giải phóng Nhân sự</span>}
      open={open}
      onCancel={onClose}
      footer={null}
      width={600}
    ...
    >
      <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-100 flex items-center gap-4">
        <Avatar size="large" className="bg-blue-600">{devName.charAt(0)}</Avatar>
        <div>
          <div className="text-sm text-gray-600">Bạn đang làm thủ tục kết thúc dự án cho:</div>
          <div className="text-lg font-bold text-blue-800">{devName}</div>
        </div>
      </div>

      <Form form={form} layout="vertical" onFinish={handleFinish}>
        <Divider orientation="left" plain>Đánh giá hiệu suất (Bắt buộc)</Divider>
